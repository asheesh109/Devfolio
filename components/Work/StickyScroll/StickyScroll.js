...
const StickyScroll = ({ contentItems = [] }) => {
  ...
  const cardLength = contentItems.length || 1;
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <motion.div
          animate={{ backgroundImage: linearGradients[activeCard % linearGradients.length] }}
          className="hidden lg:block h-60 w-80 rounded-xl bg-white sticky top-10 overflow-hidden"
        >
          {contentItems[activeCard]?.content ?? (
            <div className="h-full flex items-center justify-center text-white">No Content</div>
          )}
        </motion.div>
      </motion.div>
    </div>
